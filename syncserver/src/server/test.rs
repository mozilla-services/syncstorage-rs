...
fn limits_json_advertises_collection_overrides() {
    ...
    let json: Value = serde_json::from_str(&build_collection_limits_json(&limits)).unwrap();
    assert_eq!(
...
fn limits_json_omits_collections_when_none_overridden() {
    let json: Value =
        serde_json::from_str(&build_collection_limits_json(&ServerLimits::default())).unwrap();
