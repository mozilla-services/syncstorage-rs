...
use schema::{bso, collections, last_insert_rowid};
...
impl SqliteDb {
    ...
    async fn _create_collection(&mut self, name: &str) -> DbResult<i32> {
        ...
        diesel::insert_into(collections::table)
            ...
            .execute(&mut self.conn)
            .await?;
        let collection_id = diesel::select(last_insert_rowid())
            .first::<i64>(&mut self.conn)
            .await?
            .try_into()
            .map_err(|e| {
                DbError::internal(format!("Couldn't convert last_insert_rowid() to i32: {e}"))
            })?;
