...
use super::{
    COLLECTION_ID, COUNT, CollectionLock, EXPIRY, LAST_MODIFIED, MODIFIED, SqliteDb, TOMBSTONE,
    TOTAL_BYTES, USER_ID,
...
impl Db for SqliteDb {
    ...
    ///
    /// SQLite has no equivalent of MySQL's `SELECT ... LOCK IN SHARE MODE` /
    /// `FOR UPDATE` row-level locks; the whole database is instead
    /// serialized at the transaction level (readers don't block writers
    /// starting a deferred transaction, but only one writer transaction can
    /// commit at a time). We therefore only track the lock intent here and
    /// let SQLite's own transaction/file locking enforce the invariant.
