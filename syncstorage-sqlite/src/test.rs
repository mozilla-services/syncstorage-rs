...
use crate::{models::SqliteDb, pool::SqliteDbPool, schema::collections, DbResult};

pub fn db(settings: &SyncstorageSettings) -> DbResult<SqliteDb> {
    let _ = env_logger::try_init();
    // inherit SYNC_SYNCSTORAGE__DATABASE_URL from the env

    let pool = SqliteDbPool::new(
        settings,
        &Metrics::noop(),
        Arc::new(BlockingThreadpool::new(1)),
    )?;
    pool.get_sync()
}
