...
RUN \
    ...
    apt-get -q update && \
    apt-get -q install -y --no-install-recommends $MYSQLCLIENTPKG cmake golang-go python3-dev python3-pip python3-setuptools python3-wheel && \
...
RUN \
    ...
    apt-get -q update && \
    apt-get -q install -y build-essential $MYSQLCLIENTPKG libssl-dev libffi-dev libcurl4 python3-dev python3-pip python3-setuptools python3-wheel cargo curl jq && \
