...
use super::TokenserverSqliteDb;

#[async_trait(?Send)]
impl Db for TokenserverSqliteDb {
    ...
    async fn get_best_node(
        ...
        // SQLite has no GREATEST/LEAST; MAX/MIN act as their scalar
        // equivalents when given 2+ arguments (unlike the single-arg
        // aggregate form). LOG(x) relies on SQLite's math functions
        // extension, which is compiled in by default in modern SQLite.
    ...
    async fn insert_sync15_node(&mut self, params: params::Sync15Node) -> DbResult<bool> {
        let query = format!(
            r#"
            INSERT OR IGNORE INTO nodes (service, node, available, current_load, capacity, downed, backoff)
            VALUES (
