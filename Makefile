...
docker_run_mysql_e2e_tests:
	...
	exit_code=$$?
	docker cp mysql-e2e-tests:/mysql_integration_results.xml ${LOCAL_INTEGRATION_JUNIT_XML}
	exit $$exit_code
...
docker_run_spanner_e2e_tests:
	docker compose \
		...
		-f docker-compose.e2e.spanner.yaml \
	 	...
	 	--abort-on-container-exit; 
	exit_code=$$?
	docker cp spanner-e2e-tests:/spanner_integration_results.xml ${LOCAL_INTEGRATION_JUNIT_XML}
	exit $$exit_code
