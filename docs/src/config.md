...
| Env Var | Default Value | Description |
| --- | --- | --- |
| <span id="INIT_NODE_URL"></span>INIT_NODE_URL | None | The storage node URL, protocol + host,  to upsert into the `nodes` table on startup.  This is the origin where the service is hosted, e.g. "http://localhost:8000". |
| <span id="INIT_NODE_CAPACITY"></span>INIT_NODE_CAPACITY | 100000 | The storage node capacity of the server specified by `INIT_NODE_URL`. Only used if `INIT_NODE_URL` is set. |
