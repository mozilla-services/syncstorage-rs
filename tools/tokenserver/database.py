...
class Database:
            ...
            if cur_row.replaced_at is not None or cur_row.node is None:
                if cur_row.generation < MAX_GENERATION:
                    user = self.allocate_user(
                        email, cur_row.generation, cur_row.client_state, cur_row.keys_changed_at
                    )
    ...
    def allocate_user(
        self, email, generation=0, client_state="", keys_changed_at=0, node=None, timestamp=None
    ):
    ...
    def _build_old_user_query(self, uid_range, params, **kwargs):
        if uid_range:
            ...
            try:
                ...
                pass
            rrep = " and ".join(rstr)
            sql = sqltext(_GET_OLD_USER_RECORDS_FOR_SERVICE_RANGE.replace("::RANGE::", rrep))
        else:
            sql = _GET_OLD_USER_RECORDS_FOR_SERVICE
        return sql

    def get_old_user_records(self, grace_period=-1, limit=100, offset=0, uid_range=None):
    ...
    def add_node(self, node, capacity, **kwds):
        ...
        cols = ["service", "node", "available", "capacity", "current_load", "downed", "backoff"]
