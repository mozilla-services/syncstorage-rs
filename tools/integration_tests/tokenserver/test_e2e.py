...
@unittest.skip("Pending PyFxA oauth fix: " "https://github.com/mozilla/PyFxA/issues/101")
class TestE2e(TestCase, unittest.TestCase):
    ...
    def _get_bad_token(self):
        ...
        pem = key.private_bytes(
            encoding=serialization.Encoding.PEM, format=format, encryption_algorithm=algorithm
        )
    ...
    def test_unauthorized_oauth_error_status(self):
        ...
        headers = {"Authorization": "Unsupported-Auth-Scheme IHACKYOU", "X-KeyID": "1234-qqo"}
    ...
    def test_valid_oauth_request(self):
        ...
        self.assertEqual(res.json["api_endpoint"], "%s/1.5/%s" % (self.NODE_URL, user["uid"]))
        self.assertEqual(res.json["duration"], DEFAULT_TOKEN_DURATION)
        self.assertEqual(res.json["hashalg"], "sha256")
        self.assertEqual(res.json["hashed_fxa_uid"], self._fxa_metrics_hash(fxa_uid)[:32])
