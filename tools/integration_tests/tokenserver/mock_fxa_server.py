...
def _mock_oauth_verify(request):
    ...
    return Response(json=body["body"], content_type="application/json", status=body["status"])
...
def make_server(host, port):
    with Configurator() as config:
        ...
        config.add_view(_mock_oauth_verify, route_name="mock_oauth_verify", renderer="json")
