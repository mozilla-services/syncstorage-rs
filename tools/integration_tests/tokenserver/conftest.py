...
def _set_local_test_env_vars():
    ...
    """
    use_oauth = os.environ.get("USE_OAUTH")
...
def setup_server_end_to_end_testing():
    ...
    # debatable if this should ONLY be here since it was only done against the 
    # "run_end_to_end_tests" prior, of if we just do it in _set_local_test_env_vars...
    use_oauth = os.environ.get("USE_OAUTH")
    print("use_oauth: ", use_oauth)
    if not use_oauth:
