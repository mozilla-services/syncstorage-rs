...
class TestStorage(StorageFunctionalTestCase):
    ...
    def test_collection_usage(self):
        ...
        self.assertEquals(round(xxx_col2_size, 2), round(wanted, 2))
    ...
    def test_batch_size_limits(self):
        ...
        # }, status=400)
        # self.assertEqualss(res.json, WEAVE_SIZE_LIMIT_EXCEEDED)
        #
        ...
        # res = self.retry_post_json(endpoint, bsos)
        # self.assertEqualss(res.json['failed']['toomany'], 'retry bso')
        #
        ...
        bsos[1]["payload"] += "X"
        res = self.retry_post_json(endpoint, bsos)
        self.assertEquals(res.json["success"], ["little"])
        self.assertEquals(res.json["failed"]["big"], "retry bytes")
    ...
    def test_batch_with_failing_bsos(self):
        ...
        resp = self.retry_post_json(collection + "?batch=true", bsos)
        self.assertEquals(len(resp.json["failed"]), 1)
        self.assertEquals(len(resp.json["success"]), 1)
        batch = resp.json["batch"]
    ...
    def test_cors_settings_are_set(self):
        ...
        self.assertEquals(int(res.headers["access-control-max-age"]), 555)
        self.assertEquals(
