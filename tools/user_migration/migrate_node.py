...
class Collections:
    ...
    def __init__(self, databases):
        ...
        def transact(transaction, values):
            transaction.insert("collections", columns=("collection_id", "name"), values=values)

        self.spanner = databases["spanner"]
        try:
            # fetch existing:
            with self.spanner.snapshot() as scursor:
                rows = scursor.execute_sql("select collection_id, name from collections")
                for collection_id, name in rows:
                    logging.debug("Loading collection: {} => {}".format(name, collection_id))
                    self._by_name[name] = collection_id
            cursor.execute(sql)
            for collection_id, name in cursor:
                if name not in self._by_name:
                    logging.debug("Adding collection: {} => {}".format(name, collection_id))
                    ...
                    try:
                        ...
                        logging.info("Skipping already present collection {}".format(values))
                        pass
        finally:
            cursor.close()
    ...
    def get(self, name, collection_id=None):
        ...
        if id is None:
            logging.warn("Unknown collection {}:{} encountered!".format(name, collection_id))
...
def move_user(databases, user_data, collections, fxa, bso_num, args, report):
    ...
    ORDER BY
        bso.collection, bso.id""".format(
        bso_num
    )
    unique_key_filter = set()

    def spanner_transact_wipe_user(transaction, fxa_uid, fxa_kid, args):
    ...
    def spanner_transact_uc(transaction, data, fxa_uid, fxa_kid, args):
    ...
    def spanner_transact_bso(transaction, data, fxa_uid, fxa_kid, args):
        ...
        for col, cid, cmod, bid, exp, bmod, pay, sid in data:
            ...
            if collection_id != cid:
                logging.debug(
                    "Remapping collection '{}' from {} to {}".format(col, cid, collection_id)
                )
            # columns from sync_schema3
            ...
            count += 1
        if not args.dryrun:
            logging.debug("###bso{} {}".format(bso_num, dumper(bso_columns, bso_values)))
            for i in range(0, 5):
                try:
                    transaction.insert("bsos", columns=bso_columns, values=bso_values)
                    break
                except grpc._channel_._InactiveRpcError as ex:
                    logging.warn("Could not write record (attempt {})".format(i), exc_info=ex)
                    time.sleep(0.5)
        else:
    ...
    try:
        ...
        logging.info("Processing... {} -> {}:{}".format(uid, fxa_uid, fxa_kid))
        ...
        for row in cursor:
            ...
            if abort_col and int(row[1]) == int(abort_col):
                ...
                if col_count > abort_count:
                    logging.debug(
                        "Skipping col: {}: {} of {}".format(row[0], col_count, abort_count)
                    )
                    continue
            data.append(row)
        if args.abort:
            logging.info("Skipped {} of {} rows for {}".format(abort_count, col_count, abort_col))
        logging.info(
            "Moving {} items for user {} => {}:{}".format(len(data), uid, fxa_uid, fxa_kid)
        )
        ...
        logging.warn(
            "User {} already imported fxa_uid:{} / fxa_kid:{}".format(uid, fxa_uid, fxa_kid)
        )
        ...
        logging.error("Unexpected Batch failure: {}:{}".format(fxa_uid, fxa_kid), exc_info=ex)
    finally:
...
def get_percentage_users(users, user_percent):
    (block, percentage) = map(int, user_percent.split(":"))
    total_count = len(users)
    chunk_size = max(1, math.floor(total_count * (int(percentage) * 0.01)))
        ...
        if args.user:
            ...
            try:
                ...
                with open(bso_users_file) as bso_file:
                    line = 0
                    for row in csv.reader(bso_file, delimiter="\t"):
                        if row[0] == "uid":
                            continue
                        users.append(row)
                        tick(line)
...
def move_database(databases, collections, bso_num, fxa, args, report):
    ...
    logging.info(
        "Finished BSO #{} ({} rows) in {} seconds".format(
            bso_num, rows, math.ceil(time.time() - start)
        )
    )
    return rows
...
def get_args():
    ...
    parser.add_argument("--offset", type=int, default=0, help="UID to start at (default 0)")
    ...
    parser.add_argument(
        "--start_bso", default=0, type=int, help="start dumping BSO database (default: 0)"
    )
    parser.add_argument(
        "--end_bso", type=int, default=19, help="last BSO database to dump (default: 19)"
    )
    parser.add_argument(
        "--bso_num", type=int, help="only move this bso (equivalent to start_bso == end_bso)"
    )
    parser.add_argument(
        "--write_chunk",
        dest="chunk",
    ...
    parser.add_argument(
        ...
        default="1:100",
        help=(
            "Offset and percent of users from this BSO"
    ...
    parser.add_argument(
        "--success_file",
        default="success_{}.log".format(pid),
        help="File of successfully migrated userids",
    )
    parser.add_argument(
        "--failure_file",
        default="failure_{}.log".format(pid),
